...
class DenseDeconvolutionFunction(Function):
    @staticmethod
    def forward(
        ctx,
        input_features,
        ...
        sparseconvnet.forward_pass_multiplyAdd_count +=\
            dim_typed_fn(
                dimension, input_features, 'DenseDeconvolution_updateOutput')(
                input_spatial_size,
                output_spatial_size,
                filter_size,
                filter_stride,
    ...
    def backward(ctx, grad_output):
        ...
        dim_typed_fn(
            ctx.dimension, ctx.input_features, 'DenseDeconvolution_backward')(
...
class DenseDeconvolution(Module):
    ...
    def forward(self, input):
        ...
        input2.setLocations(input.getSpatialLocations(),torch.Tensor(input.features.data.shape))
    ...
    def __repr__(self):
        s = 'DenseDeconvolution ' + str(self.nIn) + '->' + str(self.nOut) + ' C'
